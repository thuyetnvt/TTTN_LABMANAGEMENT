...
| Xóa thiết bị | Có | Không | Không | Không | Admin; không xóa nếu đã có lịch sử mượn hoặc đang bị giữ bởi phiếu mượn. |
...
| Xử lý trả, hỏng, nhắc trả | Có | Có | Không | Không | Manager kiểm tra trả, cập nhật tình trạng và bằng chứng. |
...
Trong các trạng thái trên, API từ chối sửa, điều chuyển và xóa tài sản.
Quy tắc kiểm tra cả `BorrowRecord.EquipmentId` và các dòng
